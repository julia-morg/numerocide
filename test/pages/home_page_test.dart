...
void main() {
  ...
  setUp(() {
    settings = Settings(sound: true, vibro: true, theme: 'navy', language: 'en');
    mockSave = MockSave();
  ...
  testWidgets('HomePage displays max score, new game, and continue game buttons', (tester) async {
  ...
  testWidgets('New game button shows tutorial dialog if tutorial not passed', (tester) async {
    when(() => mockSave.isTutorialPassed()).thenAnswer((_) async => false);
    await tester.pumpWidget(
      MaterialApp(
        localizationsDelegates: AppLocalizations.localizationsDelegates,
  ...
  testWidgets('Continue game button navigates to game page if there is a saved game', (tester) async {
    Save save = Save();
  ...
  group('HomePage - _onNewGamePressed', () {
    testWidgets('Show tutorial popup. Confirm', (WidgetTester tester) async {
      ...
      expect(find.textContaining('The rules are always available in the settings menu.'), findsOneWidget);
    });
    ...
    testWidgets('Show save game popup. Confirm', (WidgetTester tester) async {
      ...
      expect(find.textContaining('Your current progress will be lost'), findsOneWidget);
